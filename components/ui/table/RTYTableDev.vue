  <template>
    <div class="rty-table-container" :class="{ 'border rounded-lg': options?.tblBorder }">
            ...
            <tbody class="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700 p-2"
              ...
              <template v-for="(row, rowIndex) in paginatedData" :key="row.id || rowIndex">
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-800">
                  <td v-for="(column, hIndex) in columns" :key="hIndex" class="px-3 py-1 whitespace-nowrap text-sm">
      ...
      {{ paginatedData.length }} / {{ pagination.totalItems }}  
...
const getData = async () => {
  ...
  try {
    ...
    showPagination.value = data.showPagination !== false; // Default to true if not specified
    ...
    console.log("Show pagination ===============: ", showPagination.value)
    ...
    // tableData.value = data || data.items || data.content || []
...
const handlePageSizeChange1111 = async (size) => {

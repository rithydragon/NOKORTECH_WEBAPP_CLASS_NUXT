    ...
    <RTModal v-if="isModalOpen" :show="isModalOpen" :mode="modalMode" :message="confirmMessage" :modalTitle="modalMode"
        ...
        <template #default="{ data }">
            <header class="w-full items-center">
                ...
                <h2 class="text-xl font-bold text-center w-full">បញ្ជីពិន្ទុសិស្ស</h2>
            <table border="1">
                <thead>
                    <tr>
                        <th>Subject</th>
...
const saveData = async (row, index) => {
    try {
        const apiUrl = row.id
            ? `/api/teaching/update/${row.id}`
            : "/api/teaching/create";

        const method = row.id ? "POST" : "POST";
...
const saveAllData = async () => {
    ...
    try {
        ...
        const {data,error} = await useHttp("/api/teaching/update_all", {
            method: "PUT",
            data:dataToSave
        });

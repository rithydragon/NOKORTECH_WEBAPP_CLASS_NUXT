...
const fetchTeachings = async () => {
    try {
        ...
        tableData.value = teachingList.map(item => ({
            id: item.Id,
...
const saveData = async (row, index) => {
    try {
        const apiUrl = row.id
            ? `http://localhost:5644/api/teaching/update/${row.id}`
            : "http://localhost:5644/api/teaching/create";
...
const saveAllData = async () => {
    ...
    try {
        ...
        const res = await useHttp("/api/teaching/update_all", {
            method: "POST",
            data:dataToSave
        });
